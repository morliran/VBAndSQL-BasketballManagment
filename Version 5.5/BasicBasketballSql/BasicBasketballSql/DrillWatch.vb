...
#Region "אופיציות של הפורם ופונקציות עזר"
    Private Sub DrillWatch_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Merror.setFormControlsEvent(Me)
...
#Region "פעולות בעת לחיצה על כפתורים או תמונות שיש בפורם"
    ...
    Private Sub Btn_Miss_Click(sender As Object, e As EventArgs) Handles Btn_Miss.Click
        ...
        End If
    End Sub
    Private Sub Btn_SystemWatch_Click(sender As Object, e As EventArgs) Handles Btn_SystemWatch.Click
        If CheckRequirments() = False Then
            MessageBox.Show("You have empty boxes", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        ElseIf NMB_DrillGoal.Value = "0" Then
            ...
            NMB_DrillGoal.Focus()
            Exit Sub
        ElseIf Chart1.Series("Success").Points.Count() <> 0 Or Chart1.Series("Failed").Points.Count() <> 0 Then
            MessageBox.Show("Please click on reset button to make another watch", "Notice", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
        If Not TimerAutoClick.Enabled Then
    ...
    Private Sub TimerAutoClick_Tick(sender As Object, e As EventArgs) Handles TimerAutoClick.Tick
        Dim m_Rnd As New Random, val As Integer = m_Rnd.Next(1, 3)
        Select Case val
            Case 1
                Chart1.Series("Success").MarkerStyle = DataVisualization.Charting.MarkerStyle.None
                If Tb_TempFailVal.Text = "0" Then
    ...
    Private Sub Pic_Reset_Click(sender As Object, e As EventArgs) Handles Pic_Reset.Click
        ...
        If TimerAutoClick.Enabled Then
            TimerAutoClick.Stop()
        End If
    End Sub
#End Region
