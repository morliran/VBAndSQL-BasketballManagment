    ...
    Public Sub getDBLogin(ByVal val As Integer, Optional ByVal Name As String = "", Optional ByVal Details() As String = Nothing) 'Name is for add new team
        ...
        Try
            ...
            Else
                ...
                LoginForm.ProgressBar1.Value = 4
                ...
                If Rder.GetString(0) = "ADMIN" Then
                    AdminsMain.Show()
                    ManagersMain.BackgroundImage = AdminsMain.BackgroundImage
                Else
                    ...
                    ManagersMain.BackgroundImage = My.Resources.ResourceManager.GetObject(PicName) 'הצבה של לוגו הקבוצה כתמונת רקע
                    ManagersMain.Show()
                End If
                LoginForm.Hide()
            End If
...
#Region "פונקציה אשר ממלאה את תיבות הבחירה הדורשות מידע ממסד הנתונים"
    Public Sub FillBoxes(ByVal Combo As ComboBox, ByVal Str As String, Optional ByVal QueryCode As Integer = 0)
        Try
            Dim Drow As DataRow
            CN.Open()
            C.GetAdapter.SelectCommand = New SqlCommand()
            C.GetAdapter.SelectCommand.Connection = CN
            C.GetAdapter.SelectCommand.Parameters.AddWithValue("val", Str)
            If QueryCode <> 0 Then
        ...
        Try
            ...
            If Rder.Read Then
                Select Case frm.Name
                    ...
                    Case "UpdatePlayersForm"
                        UpdatePlayersForm.Tb_PID.Text = Trim(Rder.GetString(1))
                        UpdatePlayersForm.Tb_FN.Text = Trim(Rder.GetString(2))
                        UpdatePlayersForm.Tb_LN.Text = Trim(Rder.GetString(3))
                        UpdatePlayersForm.Tb_JN.Text = Trim(Rder.GetString(4))
                        UpdatePlayersForm.CB_Position.Text = Trim(Rder.GetString(5))
                        UpdatePlayersForm.CB_SPosition.Text = Trim(Rder.GetString(6))
                        UpdatePlayersForm.MB_Height.Text = Trim(Rder.GetDouble(7))
                        UpdatePlayersForm.MB_DOB.Text = Trim(Rder.GetDateTime(8)) 'כדי לקבל את התאריך שמופיע במסד הנתונים'
                        UpdatePlayersForm.Tb_HTC.Text = Trim(Rder.GetString(9))
                        UpdatePlayersForm.TB_ATC.Text = Trim(Rder.GetString(10))
                    Case "DrillWatch"
                        DrillWatch.Lbl_Code.Text = Trim(Rder.GetString(0))
                        DrillWatch.Lbl_Name.Text = Trim(Rder.GetString(2)) & " " & Trim(Rder.GetString(3))
                    Case "StatsCompares"
                        If Combo.Name = "CB_Playercode" Then
                            StatsCompares.Tb_Playername.Text = Trim(Rder.GetString(2)) & " " & Trim(Rder.GetString(3))
                        End If
                        If Combo.Name = "CB_Playercode2" Then
                            StatsCompares.Tb_Playername2.Text = Trim(Rder.GetString(2)) & " " & Trim(Rder.GetString(3))
                        End If
                End Select
            End If
        Catch ex As Exception
...
#Region "פונקציה אשר מחזירה טבלה מתוך מסד הנתונים וממלאה אותה ואת תיבות הבחירה"
    Public Function FillDataTableAndBoxs(ByVal MyTable As DataTable, ByVal MyDs As DataSet, ByVal Combo As ComboBox, _
                                         ByVal Combo2 As ComboBox, ByVal Str As String) As DataTable
        Try
            Dim Drow As DataRow
            CN.Open()
            C.GetAdapter.SelectCommand = New SqlCommand()
            ...
            If Combo.Name = "CB_Playercode" AndAlso Combo2.Name = "CB_Playercode2" Then
                C.GetAdapter.SelectCommand.Parameters.AddWithValue("num", 4)
            ElseIf Combo.Name = "CB_Gametype" AndAlso Combo2.Name = "CB_Gametype2" Then
...
#Region "פונקציה שממלאה את הגרפים שנמצאים בפורם של ההשוואות הסטטיסטיות בין השחקנים"
    Public Sub FillChartLabels(ByVal Name As System.Windows.Forms.DataVisualization.Charting.Chart, ByVal SeriesName As String, _
        ...
        Finally
            CloseResources()
        End Try
        Name.Series(SeriesName).Points.AddXY(LabelsName(0), LabelArray(0).Text)
        Name.Series(SeriesName).Points.AddXY(LabelsName(1), LabelArray(1).Text)
        Name.Series(SeriesName).Points.AddXY(LabelsName(2), LabelArray(2).Text)
        Name.Series(SeriesName).Points.AddXY(LabelsName(3), LabelArray(3).Text)
        Name.Series(SeriesName).Points.AddXY(LabelsName(4), LabelArray(4).Text)
    End Sub
#End Region
#Region "פונקציה שמחזירה רשימה אשר מכילה ערכים מסוימיים מתוך מסד הנתונים"
    Public Function FillListView(ByVal frm As Form, ByVal myListView As ListView, ByVal itemcoll() As String, _
                                 ByVal Choice As String) As ListView
        ...
        Try
            ...
            Select Case frm.Name
                ...
                Case "PlayersUpcomingBirthday"
                    C.GetAdapter.SelectCommand.CommandText = "spUpcomingBirthday"
                Case "AddPlayersForm", "DrillWatch"
                    C.GetAdapter.SelectCommand.CommandText = "spGetSelected"
                    If frm.Name.Equals("AddPlayersForm") Then
                        C.GetAdapter.SelectCommand.Parameters.AddWithValue("num", 4)
    ...
    Public Function getDataView(ByVal Str As String, ByVal arr() As String, Optional ByVal code As Integer = 0, _
        ...
        Try
            ...
            If code = 0 AndAlso code2 = 0 AndAlso code3 = 0 Then
                Select Case Str
                    Case "Jerseynum", "Height", "DOB", "LeagueGames", "MyTeamGames", "AwayOrHomeGames", "Ages", "Pgrades", "Vis", "SubsCode"
                        C.GetAdapter.SelectCommand.Parameters.AddWithValue("val1", arr(0))
                        ...
                        End If
                    Case "Position", "SecondPosition", "BothPosition"
                        C.GetAdapter.SelectCommand.Parameters.AddWithValue("rule1", arr(2))
                        C.GetAdapter.SelectCommand.Parameters.AddWithValue("check1", arr(1))
                        C.GetAdapter.SelectCommand.Parameters.AddWithValue("rule2", arr(3))
                        C.GetAdapter.SelectCommand.Parameters.AddWithValue("check2", arr(4))
                        C.GetAdapter.SelectCommand.CommandText = "spBuildPosAndSeconPosDynamic"
                    Case "CheckDate"
                        C.GetAdapter.SelectCommand.Parameters.AddWithValue("val1", arr(0))
...
#Region "פונקציה להכנסת נתונים שנמצאים בפורם מסויים אל טבלה שאליה הוא מתייחס"
    Public Sub InsertToTable(ByVal frm As Form)
        Try
            ...
            Select Case frm.Name
                Case "PlayersMatchStatistics"
                    Cmd.Parameters.AddWithValue("@d1", Trim(PlayersMatchStatistics.CB_PlayerCode.Text))
                ...
                Case "AddPlayersForm"
                    Cmd.Parameters.AddWithValue("@d1", Trim(AddPlayersForm.Tb_PlayerCode.Text))
                    Cmd.Parameters.AddWithValue("@d2", Trim(AddPlayersForm.Tb_PID.Text))
                    Cmd.Parameters.AddWithValue("@d3", Trim(AddPlayersForm.Tb_FN.Text))
                    Cmd.Parameters.AddWithValue("@d4", Trim(AddPlayersForm.Tb_LN.Text))
                    Cmd.Parameters.AddWithValue("@d5", Trim(AddPlayersForm.Tb_JN.Text))
                    Cmd.Parameters.AddWithValue("@d6", Trim(AddPlayersForm.CB_Position.Text))
                    Cmd.Parameters.AddWithValue("@d7", Trim(AddPlayersForm.CB_SPosition.Text))
                    Cmd.Parameters.AddWithValue("@d8", Trim(AddPlayersForm.MB_Height.Text))
                    ...
                    Cmd.Parameters("@d9").Value = Trim(AddPlayersForm.MB_DOB.Text)
                    Cmd.Parameters.AddWithValue("@d10", Trim(AddPlayersForm.Tb_HTC.Text))
                    Cmd.Parameters.AddWithValue("@d11", Trim(AddPlayersForm.TB_ATC.Text))
                    Cmd.Parameters.AddWithValue("@d12", Trim(AddPlayersForm.Tb_RoutionCVal.Text))
                    Cmd.Parameters.AddWithValue("@d13", Trim(AddPlayersForm.Lbl_ManTeam.Text))
...
#Region "פונקציה לעדכון נתונים בטבלה שאליה הפורם מתייחס"
    Public Sub UpdateTable(ByVal frm As Form, ByVal arr() As String)
        Try
            CN.Open()
            Cmd = New SqlCommand()
            Cmd.Connection = CN
