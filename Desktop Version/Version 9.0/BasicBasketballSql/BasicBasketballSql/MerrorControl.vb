...
#Region "הצבת איוונטים ובדיקתם על האובייקטים"
    Private Sub ClearLists() 'Remove event handler from all lists items, and after that clearing all the items
        ...
        For j = 0 To Combo.Count - 1
            Combo(j).Text = TextCombo(j) 'Making sure that control text will be his default
        ...
        Combo.Clear()
    End Sub
    Private Sub AddEvents() 'Adding event handlers for all lists items, and default text value for the hint property
        Dim i As Integer, j As Integer
        For i = 0 To Text.Count - 1
