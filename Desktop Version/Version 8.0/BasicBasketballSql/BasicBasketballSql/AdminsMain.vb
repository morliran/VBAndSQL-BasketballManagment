...
#Region "פוקנציות שפועלות על הפורם עצמו"
    ...
    Private Sub Panel_Display_Resize(sender As Object, e As EventArgs) Handles Panel_Display.Resize
        CenterAllDispPanels()
    End Sub
    Private Sub Panel_Top_Resize(sender As Object, e As EventArgs) Handles Panel_Top.Resize
        Lbl_WelcomeAdmin.Location = New Point((Panel_Top.Width - Lbl_WelcomeAdmin.Width) / 2, (Panel_Top.Height - Lbl_WelcomeAdmin.Height) / 2)
    End Sub
    ...
    Private Sub LabelsMouseEnters(sender As Object, e As EventArgs) Handles Lbl_TeamsAdd.MouseEnter, Lbl_TeamsUpdate.MouseEnter, Lbl_PlayersAdd.MouseEnter, Lbl_PlayersUpdate.MouseEnter, Lbl_PlayersDelete.MouseEnter, Lbl_GamesAdd.MouseEnter, Lbl_GamesUpdate.MouseEnter, Lbl_GamesAddStats.MouseEnter, Lbl_LeagueLead.MouseEnter, Lbl_LeaguePChanges.MouseEnter, Lbl_LeagueSChanges.MouseEnter, Lbl_LeagueGChanges.MouseEnter
        ...
        label.BackColor = ColorTranslator.FromHtml("#F65058")
    End Sub
...
#Region "פונקציות עזר"
    ...
    Private Sub TakeManageInformation(ByVal str As String)
        ...
        If str = "Players" Then
            ...
            FillTeamsNamesBox(CB_TeamsName)
    ...
    Private Function CheckVailed(ByVal val As Integer) As Boolean
        If val = 1 Then
            If CB_GameCode.Text.Equals("Game code") Or Tb_HostersCval.Text.Equals("Home code") Or Tb_VisitorsCval.Text.Equals("Away code") Or MTB_TimeVal.Text.Equals("") Or MTB_GameDate.Text.Equals("") Then
        ...
        Return False
    End Function
    Private Sub SetDeleteDataView(ByVal code As String)
        Dim Arr() As String = New String() {"HisHomeTeamCode", "=", code, "", "", ""}, intHeight As Integer
        DataGridView2.Columns.Clear()
        DataGridView2.DataSource = myHelper.getDataView("Players", Arr, 2)
        DataGridView2.Columns(10).Visible = False
        DataGridView2.Columns(9).Visible = False
        For Each rowheight As DataGridViewRow In DataGridView2.Rows 'Change the size of the datagrid to the size of the table that returns from the database
...
#Region "אירועים שיופעלו בעת לחיצה על אופציה שבסרגל הבחירה"
    ...
    Private Sub Lbl_GamesAdd_Click(sender As Object, e As EventArgs) Handles Lbl_GamesAdd.Click
        ...
        If ChekerVal.Equals("6") Then 'If there is 6 game in the max game type, increase game type
...
#Region "לחיצות על התמונות או הכפתורים שיש בפאנלים"
        ...
        If StartManaging() = True Then
            ...
            Select Case Lbl_WhatManage.Text
                ...
                Case "DeleteP"
                    ...
                    Lbl_PDeleteTeamName.Text = "Delete player from " & ManagersMain.getManager.CheckTeam()
                    SetDeleteDataView(ManagersMain.getManager.GetHomeCode())
                Case "AddPlayerStats"
                    If CB_Gametype.Text <> "Game type" AndAlso CB_TeamsName.Text <> "League teams names" AndAlso Tb_TGameCval.Text <> "Game code" AndAlso Tb_GameDate.Text <> "Game date" Then
    ...
    Private Sub Pic_ManageTeamsPic_Click(sender As Object, e As EventArgs) Handles Pic_ManageTeamsPic.Click
        Dim NameCheck As String = myHelper.getColumn("TeamName", Tb_TeamN.Text) 'If there is already existing team name
        ...
        If Pic_ManageTeamsPic.Tag = "Update" Then
            If Tb_ArenaDetail.Text.Equals("") Or Tb_ArenaDetail.Text.Equals("Arena name") Then
                MsgBox("Unvailed arena name")
                Exit Sub
            End If
            If CHB_TeamNChange.Checked = True AndAlso (Tb_TeamN.Text.Equals("Team name") Or Tb_TeamN.Text.Equals("")) Then
...
#Region "פונקציות למילוי תיבות הבחירה או רשימות הבחירה"
    ...
    Public Sub UsersSubString(ByVal s As String, ByVal n As Integer)
        ...
        For i = 0 To CB_TeamUpdate.Items.Count - 1
            CheckUser = myHelper.getColumn("MyName", CB_TeamUpdate.Items.Item(i).ToString()) 'If there is already existing username
            If CheckUser IsNot Nothing Then
                LB_UserOptions.Items.Remove(CheckUser) 'Delete him so the system will not allowed duplicate username
            End If
        Next
    End Sub
#End Region
#Region "פונקציה לקבלת קיצור שמות הקבוצות לשימוש בפורומים של ניהול קבוצות או שחקנים"
    Public Sub setTeamShortCut(ByVal code As String)
        ...
        PicName = LoginForm.getManagerTeamName().Replace("-", "") 'Matching team name to her picture name
...
#Region "איוונטים על אובייקטים שנמצאים בפאנלים"
    ...
    Private Sub Tb_TeamN_Leave(sender As Object, e As EventArgs) Handles Tb_TeamN.Leave
        Dim index As Integer, cnt As Integer = 0, Fchar As Char, SecChar As Char, LChar As Char, FirstLetters As String = ""
        If Tb_TeamN.Text <> "Team name" AndAlso Tb_TeamN.Text <> "" AndAlso Tb_TeamN.Text <> " " Then 'Making function avoid working on default text value of the textbox or on empty text
            Tb_TeamN.Text = Tb_TeamN.Text.Replace(" ", "-")
            Tb_TeamN.Text = Tb_TeamN.Text.Substring(0, 1).ToUpper() + Tb_TeamN.Text.Substring(1).ToLower() 'First letter will be Upper case and the others lower case
    ...
    Private Sub Help_Click(sender As Object, e As EventArgs) Handles Pic_ManageTHelp.Click, Pic_GameHelp.Click, Btn_DHelp.Click
        HelpForm.Show()
    End Sub
    Private Sub TextBox_TextChanged(sender As Object, e As EventArgs) Handles TB_MTeamTemp.TextChanged, TB_GaTemp.TextChanged, TB_PTemp.TextChanged
        Dim tb As TextBox = sender
        tb.Clear()
    End Sub
    Private Sub CB_TeamsName_TextChanged(sender As Object, e As EventArgs) Handles CB_TeamsName.TextChanged
        If Lbl_WhatManage.Text = "AddPlayerStats" Then
