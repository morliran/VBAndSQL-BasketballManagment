...
#Region "פונקציות על הפורם"
    Private Sub PlayersMatchStatistics_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If CountPlayers = Integer.Parse(Lbl_CheckTotal.Text) Then 'Prevent the option to add or update player stats after finish insert him
            ...
            End If
        Else
            If CountPlayers <> 0 Then 'If this is not the first time that the manager is inserting his players stats on game date
                ManagersMain.getDBHelper.FillBoxes(CB_AlreadyHaveCode, ArrCheck(0), 3, CB_AlreadyHaveName, ArrCheck(1))
                DeleteExistingStatsPlayers(CB_PlayerMatchCode, CB_AlreadyHaveCode)
                DeleteExistingStatsPlayers(CB_PMatchNames, CB_AlreadyHaveName)
                Lbl_CheckTotal.Text = CB_PlayerMatchCode.Items.Count
            End If
        End If
        Accessibility.SetDesignToAllForms(Me)
    End Sub
    Private Sub DeleteExistingStatsPlayers(ByVal MainCombo As ComboBox, ByVal TempCombo As ComboBox)
