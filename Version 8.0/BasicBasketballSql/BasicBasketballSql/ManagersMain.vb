...
Public Class ManagersMain
    ...
    Private RowSize As Integer = 10, CntMaxRoutionCode As Integer = 0, CntFileLines As Integer = 0, RowFontName As String = "", RowFontStyle As FontStyle = FontStyle.Regular, RowFontC As String = "", OddRowColor As String = "", EvenRowColor As String = ""
...
#Region "פונקציות עזר"
    ...
    Public Sub DesignDataGrid(ByVal DGV As DataGridView)
        Dim FirstC As String = "", SecondC As String = "", ind As Integer = 0
        DGV.Visible = True
        ClearPanelText()
        If DGV.Columns.Contains("Height") Then
            DGV.Columns("Height").DefaultCellStyle.Format = "N2" 'Format height column values as #.##
        End If
...
#Region "Design button options"
    ...
    Private Sub Lbl_DesignDarkLightMode_Click(sender As Object, e As EventArgs) Handles Lbl_DesignDarkLightMode.Click
        Merror.AllowRootControls(Of Panel)(Panel_Grid, Panel_TopGrid.Name)
        If Lbl_DesignDarkLightMode.Text.Equals("Dark Mode") Then
...
#Region "קריאה וכתיבת לקובץ של העיצוב לפי צבעי הקבוצה"
    Private Sub WriteFileValues(ByVal FirstColor As String, ByVal SecondColor As String)
        Using FWriter As StreamWriter = My.Computer.FileSystem.OpenTextFileWriter(TextFilesPath & "/" & Manager.CheckTeam() & " Colors.txt", False)
    ...
    Private Sub ReadFileValues()
        ...
        If CntFileLines = 2 Then
            Using reader As New StreamReader(TextFilesPath & "/" & Manager.CheckTeam() & " Colors.txt")
    ...
    Private Sub SetTheTeamsColors()
        ...
        End While
        If cnt = 2 Then
            If TeamFirstC.Equals("#FFFFFF") Or TeamFirstC.Equals("White") Then
                Temp = TeamFirstC
                TeamFirstC = TeamSecondC
                TeamSecondC = Temp
                If (TeamFirstC.Equals("#FFFFFF") Or TeamFirstC.Equals("White")) AndAlso (TeamSecondC.Equals("#FFFFFF") Or TeamSecondC.Equals("White")) Then
                    MessageBox.Show("Sorry, cannot saving two white colors" & vbNewLine & "Here is the default color table", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
