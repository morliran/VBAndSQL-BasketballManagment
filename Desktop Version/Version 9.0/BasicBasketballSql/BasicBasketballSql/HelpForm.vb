...
Public Class HelpForm
    Private MyErrorHelp As MerrorControl = New MerrorControl()
...
#Region "פונקציות של הפורם"
    Private Sub HelpForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim TreeP As TreeView, n As TreeNode, nChild As TreeNode, HebControl() As Control = New Control() {Tb_SearchHeb, TreeV_HelpHeb, RTB_TempHeb}, EngControl() As Control = New Control() {Tb_SearchEng, TreeV_HelpEng, RTB_TempEng}
        MyErrorHelp.SetWantsTime(False)
        Accessibility.SetDesignToAllForms(Me)
        If Directory.Exists(TextFilesPath) = False Then
            MessageBox.Show("The system cant find the help text in Debug folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Me.Close()
        End If
        If Accessibility.Language.Equals("Hebrew") Then
            MyErrorHelp.SetControlsVisibilty(HebControl, True)
            MyErrorHelp.SetControlsVisibilty(EngControl, False)
            Panel_Right.BackColor = Me.BackColor
            Panel_Left.BackColor = Color.LightSkyBlue
            RTB_TempHeb.BackColor = Panel_Left.BackColor
        Else
            MyErrorHelp.SetControlsVisibilty(HebControl, False)
            MyErrorHelp.SetControlsVisibilty(EngControl, True)
